...
public class ServerGUI : MonoBehaviour {
	...
	void OnGUI(){
		...
		if(menu == Menu.general){
			...
			core.serverName = GUI.TextField(new Rect(settingsLeftIndent + settingsWidth - settingsWidth / 2 - labelWidth, settingsTopIndent + 225, 200, 20), core.serverName);
			...
			int.TryParse(GUI.TextField(new Rect(settingsLeftIndent + settingsWidth - settingsWidth / 2 - labelWidth, settingsTopIndent + 280, textfieldIntWidth, 20), core.port.ToString()), out core.port);
			...
			//Gamemode Selection Dropdown
			GUI.Label(new Rect(settingsLeftIndent + settingsWidth - settingsWidth / 2 - labelWidth, settingsTopIndent + 200, 200, 25), "Choose gamemode:", labelText);
			gamemodeSelectionRect = new Rect(settingsLeftIndent + settingsWidth - settingsWidth / 2 - 2, settingsTopIndent + 225,125, gamemodeSelectionList.Length * 25);
			if(GUI.Button(new Rect((gamemodeSelectionRect.x - 100), gamemodeSelectionRect.y, 125, 25), "")){
			...
			if(showGamemodeSelection){
				...
				for(int index = 0; index < gamemodeSelectionList.Length; index++)
				{
					...
					if(GUI.Button(new Rect(0, (index*25), gamemodeSelectionRect.width, 25), ""))
					{
						showGamemodeSelection = false;
					...
					GUI.Label(new Rect(5, (index*25), gamemodeSelectionRect.width, 25), gamemodeSelectionList[index]);
					
				}
				
				GUI.EndScrollView();   
			} else {
				GUI.Label(new Rect((gamemodeSelectionRect.x - 95), gamemodeSelectionRect.y, 300, 25), gamemodeSelectionList[gamemodeSelectionIndexNumber]);
			}
			...
			int.TryParse(GUI.TextField(new Rect(settingsLeftIndent + settingsWidth - settingsWidth / 2 - labelWidth, settingsTopIndent + 225, textfieldIntWidth, 20), core.maxPlayers.ToString()), out core.maxPlayers);
		}
			...
			//Startbutton
			if(GUI.Button(new Rect(settingsLeftIndent + settingsWidth - settingsWidth / 2 - labelWidth - 1, settingsTopIndent + 435, 200, 40), "Start Server")){
				core.StartServer();
				serverStarted = true;
				menu = Menu.overview;
			}
			...
			//Stopbutton
			if(GUI.Button(new Rect(settingsLeftIndent + settingsWidth - settingsWidth / 2 - labelWidth - 1, settingsTopIndent + 390, 200, 40), "Stop Server")){
				core.StopServer();
				serverStarted = false;
				menu = Menu.start;
			}
